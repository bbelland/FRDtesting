...
class FRDsolver(object):
    ...
    def residual_calculate(self,imagetosolve,guessimage,varianceimage):
        ...
        #residualval += np.sum(np.divide(np.square(currentimage-modeltocompare),(modeltocompare)))*np.sqrt(2)
        #residualval -= np.sum(np.divide(np.square(currentimage-modeltocompare),(modeltocompare)))*np.sqrt(2)
    
        """residualval += np.sum(np.divide(np.square(currentimage[(centerx-3):(centerx+3),(centery-3):(centery+3)]-modeltocompare[(centerx-3):(centerx+3),(centery-3):(centery+3)]),(modeltocompare[(centerx-3):(centerx+3),(centery-3):(centery+3)])))*np.sqrt(2)
        residualval -= np.sum(np.divide(np.square(currentimage[(centerx-1):(centerx+1),(centery-1):(centery+1)]-modeltocompare[(centerx-1):(centerx+1),(centery-1):(centery+1)]),(modeltocompare[(centerx-1):(centerx+1),(centery-1):(centery+1)])))*np.sqrt(2)"""
        ...
        plt.imshow(currentimage[(centery-3):(centery+3),(centerx-3):(centerx+3)] - modeltocompare[(centery-3):(centery+3),(centerx-3):(centerx+3)])
        plt.colorbar()
        plt.title('Difference image between input and comparison')
        plt.show()        
        
        #plt.imshow(np.sqrt(2)*np.divide(np.square(currentimage[(centery-3):(centery+3),(centerx-3):(centerx+3)]-modeltocompare[(centery-3):(centery+3),(centerx-3):(centerx+3)]),(varimage[(centery-3):(centery+3),(centerx-3):(centerx+3)]))):
        #plt.title('Chi square in each pixel')
        #plt.show()
        
        #print('Added to residual val: ')
        
        residualval += np.sum(np.divide(np.square(currentimage[(centery-3):(centery+3),(centerx-3):(centerx+3)]-modeltocompare[(centery-3):(centery+3),(centerx-3):(centerx+3)]),(varianceimage[(centery-3):(centery+3),(centerx-3):(centerx+3)])))*np.sqrt(2)
        
        residualval -= np.sum(np.divide(np.square(currentimage[(centery-1):(centery+1),(centerx-1):(centerx+1)]-modeltocompare[(centery-1):(centery+1),(centerx-1):(centerx+1)]),(varianceimage[(centery-1):(centery+1),(centerx-1):(centerx+1)])))*np.sqrt(2)
    ...
    def find_FRD_compare_positions(self, FRDlist, knownimagelist, imagetosolve,varianceimage):
        ...
        for FRDindex in range(len(FRDlist)):
            #plt.imshow(knownimagelist[FRDindex]-imagetosolve)
            #absmax = np.max([np.max(knownimagelist[FRDindex]-imagetosolve),-np.min(knownimagelist[FRDindex]-imagetosolve)])
            #plt.clim([-absmax,absmax])
            #plt.colorbar()
            #plt.show()
